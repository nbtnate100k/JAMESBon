...
async def tg_start(update, context) -> None:
    ...
    text = (
        ...
        "If commands say Not authorized, set <code>OWNER_TELEGRAM_ID</code> on the server to this id "
...
async def tg_stock(update, context) -> None:
    ...
    if len(parts) < 3:
        await msg.reply_text("Usage: /stock <price> <card_data>\nMultiple cards: separate with ;;")
        return
    try:
        price = float(parts[1])
    except ValueError:
    ...
    with state_lock:
        ...
        save_state()
    await msg.reply_text(f"Added {added} card(s) at ${price:.2f} each. Website loads via /api/products.")
    ...
    async def post_init(app) -> None:
        ...
        await app.bot.set_my_commands(
            [
                ...
                BotCommand("stock", "Add shop stock"),
