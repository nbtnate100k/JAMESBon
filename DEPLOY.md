...
**Deploy failed / healthcheck:** If the build log shows the start command as random text instead of `gunicorn`, Nixpacks mis-read a **comment** in `Procfile`. This repo uses a comment-free `Procfile` plus `nixpacks.toml` `[start]` and `railway.toml` `startCommand` so the web server always starts.
